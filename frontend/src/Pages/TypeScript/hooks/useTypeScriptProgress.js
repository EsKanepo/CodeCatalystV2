...
const TOTAL_WEEKS = 6; // TypeScript = 6 minggu
const COURSE_ID = 8; // TypeScript Essentials

export const useTypeScriptProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching TypeScript progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save TypeScript progress to DB:', error);
    }
  };
