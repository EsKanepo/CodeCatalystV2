...
const TOTAL_WEEKS = 8; // CSS = 8 minggu
const COURSE_ID = 2; // CSS Styling & Layout
...
export const useCssProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching CSS progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save CSS progress to DB:', error);
    }
  };
