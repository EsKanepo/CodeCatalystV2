...
const TOTAL_WEEKS = 4; // Docker = 4 minggu
const COURSE_ID = 10; // Docker Containerization

export const useDockerProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching Docker progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save Docker progress to DB:', error);
    }
  };
