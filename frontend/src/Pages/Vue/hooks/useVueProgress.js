...
const TOTAL_WEEKS = 8; // Vue.js = 8 minggu
const COURSE_ID = 7; // Vue.js Frontend

export const useVueProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching Vue progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save Vue progress to DB:', error);
    }
  };
