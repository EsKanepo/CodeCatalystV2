...
const TOTAL_WEEKS = 3; // Bootstrap = 3 minggu
const COURSE_ID = 4; // Bootstrap Framework
...
export const useBootstrapProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching Bootstrap progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save Bootstrap progress to DB:', error);
    }
  };
