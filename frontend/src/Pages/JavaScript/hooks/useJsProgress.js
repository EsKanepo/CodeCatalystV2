...
const TOTAL_WEEKS = 7; // JavaScript = 7 sections (minggu 1-2, 3, 4, 5, 6, 7, 8)
const COURSE_ID = 3; // JavaScript Dasar
...
export const useJsProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching JS progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save JS progress to DB:', error);
    }
  };
