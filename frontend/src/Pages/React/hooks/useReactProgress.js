...
const TOTAL_WEEKS = 5; // React = 5 sections (minggu 1-2, 3-4, 5-6, 7, 8)
const COURSE_ID = 5; // React Development
...
export const useReactProgress = () => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        console.log('API error fetching React progress:', error);
        setCompletedSections(Array(TOTAL_WEEKS).fill(false));
      } finally {
        setIsLoaded(true);
      }
  ...
  const saveProgressToDb = async (newCompletedSections) => {
    ...
    try {
      ...
      console.error('Failed to save React progress to DB:', error);
    }
  };
